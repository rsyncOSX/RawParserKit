...
public enum NikonMakerNoteParser {
    ...
    public nonisolated static func embeddedJPEGLocations(from url: URL) -> NEFEmbeddedJPEGLocations? {
        guard let fh = try? FileHandle(forReadingFrom: url) else { return nil }
        defer { try? fh.close() }
    ...
    public nonisolated static func embeddedJPEGLocationsDiagnostics(from url: URL) -> RawParserDiagnostics<NEFEmbeddedJPEGLocations> {
        var trace: [String] = []
        guard let fh = try? FileHandle(forReadingFrom: url) else {
            let failure = "could not open file for reading"
            return .init(value: nil, trace: ["ERROR: \(failure)"], failure: failure)
